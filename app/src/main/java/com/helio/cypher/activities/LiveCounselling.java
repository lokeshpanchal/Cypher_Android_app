...
import com.helio.cypher.adapters.ChatDetailsAdapter;
import com.helio.cypher.appCounsellingDTO.AppointmentAutoCancelDTO;
import com.helio.cypher.appCounsellingDTO.AppointmentCompleteDTO;
import com.helio.cypher.appCounsellingDTO.ChatCounsellingMessageDTO;
import com.helio.cypher.appCounsellingDTO.ChatMessageReqDTO;
import com.helio.cypher.appCounsellingDTO.CommonRequestTypeDTO;
import com.helio.cypher.appCounsellingDTO.FinalObjectDTO;
import com.helio.cypher.appCounsellingDTO.GetChatObjectDTO;
import com.helio.cypher.appCounsellingDTO.InsertRatingDTO;
import com.helio.cypher.appCounsellingDTO.RattingQuestionDTO;
import com.helio.cypher.appCounsellingDTO.SendAttandanceDataDTO;
import com.helio.cypher.appCounsellingDTO.SendCounsellorRatingDTO;
import com.helio.cypher.appCounsellingDTO.SendMessageDTO;
import com.helio.cypher.appCounsellingDTO.SendMessageDataDTO;
import com.helio.cypher.appCounsellingDTO.SendMessageObjectDTO;
import com.helio.cypher.appCounsellingDTO.UploadMoodgraphDataDTO;
...
import com.helio.cypher.models.GetImpactRatingDTO;
...
import com.helio.cypher.models.SendImpactRatingObjectDTO;
            ...
            public void onClick(View v)
            {
                ...
                try {
                    if (user_rating > 0)
                    {
                        ...
                        /*Rating_boject.put("rattingcount", user_rating);
                        Rating_boject.put("status", "Done");

                        ParseACL user_session = new ParseACL();
                        user_session.setPublicWriteAccess(true);
                        user_session.setPublicReadAccess(true);
                        Rating_boject.setACL(user_session);
                        progress_bar.setVisibility(View.VISIBLE);

                        Rating_boject.saveInBackground(new SaveCallback() {
                            @Override
    ...
    public static void UserAttandace()
    {
        try {
          ...
          /*  if (booking_object != null) {
                ...
                String attandace = booking_object.getString("userCameAt");
                if (attandace == null || attandace.equalsIgnoreCase("")) {
                    String counseldate_time = booking_object.getString("appointmentdate");

                    String appint_array[] = counseldate_time.split(" ");
                    String date = "";
                    if (appint_array != null && appint_array.length > 0) {
                        date = appint_array[0];
                    }
                    ...
                    date = getGMTTime(date);

                    booking_object.put("userCameAt", date);
                ...
                ratingok.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        UserAttandace();
                    }
...
/*
    ...
    public static void ImpactRatingdone() {
        ...
        chatFriend.put("message", "ImpactRatingDone");
    ...
    private class GetAllmessage extends android.os.AsyncTask<String, String, Bitmap> {
        ...
        protected Bitmap doInBackground(String... args) {
            try {
                ...
                {
                    ChatCounsellingMessageDTO chatMessageDTO = new ChatCounsellingMessageDTO(appointmentid, last_msg_datetime);
                                   ...
                                   /* if (UserInfoobj.has("created_at")) {
                                        try {
                                            ...
                                            try {
                                                ...
                                                Date myDate  =   Utils.StringTodate(UserInfoobj.getString("created_at"));
                                                current = Calendar.getInstance();
                                                TimeZone tzCurrent = current.getTimeZone();
                                                int offset = tzCurrent.getRawOffset();
                                                if (tzCurrent.inDaylightTime(new Date())) {
...
*/
                                    ...
                                    me.setYourMsg(CryptLib.decrypt(messageFetch));
                                    me.setSentTime(systemDate);
